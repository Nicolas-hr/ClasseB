...
$username = filter_input(INPUT_POST, 'username', FILTER_SANITIZE_STRING, FILTER_NULL_ON_FAILURE);
$desc = filter_input(INPUT_POST, 'description', FILTER_SANITIZE_STRING, FILTER_NULL_ON_FAILURE);
...
if (filter_input(INPUT_POST, 'update')) {
    ...
    if ($username != null) {
        $updateUsername = $bdd->prepare("UPDATE Tbl_User SET Txt_Username = :updatedUsername WHERE Id_User = :id");
        $updateUsername->execute(array(':updatedUsername' => $username, ':id' => $id));
    }

    if ($desc != null) {
        $updateDesc = $bdd->prepare("UPDATE Tbl_User SET Txt_Description_Profil = :updatedDesc WHERE Id_User = :id");
        $updateDesc->execute(array(':updatedDesc' => $desc, ':id' => $id));
    }
...
if (filter_input(INPUT_POST, 'resetImage')) {
    $updateImage = $bdd->prepare("UPDATE Tbl_User SET Txt_Image_Profil = :image WHERE Id_User = :id");
    $updateImage->execute(array(':image' => 'blank-profile-picture.png', ':id' => $id));
