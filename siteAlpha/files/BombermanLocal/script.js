...
function commandesJoueurs(event) {
    ...
    switch (event.keyCode) {
        case __toucheW:
            if (getCellSols(playerWASD.joueurX, playerWASD.joueurY - 1).traversable && getElementDynamique(playerWASD.joueurX, playerWASD.joueurY - 1).traversable) {
                depHaut(playerWASD);
            }
            break;

        case __toucheHaut: // Haut
            if (getCellSols(playerUDLR.joueurX, playerUDLR.joueurY - 1).traversable && getElementDynamique(playerUDLR.joueurX, playerUDLR.joueurY - 1).traversable) {
                depHaut(playerUDLR);
            }
            break;
        ...
        case __toucheA:
            if (getCellSols(playerWASD.joueurX - 1, playerWASD.joueurY).traversable && getElementDynamique(playerWASD.joueurX - 1, playerWASD.joueurY).traversable) {
                depGauche(playerWASD);
            }
            break;

        case __toucheGauche:
            if (getCellSols(playerUDLR.joueurX - 1, playerUDLR.joueurY).traversable && getElementDynamique(playerUDLR.joueurX - 1, playerUDLR.joueurY).traversable) {
